...
def main_worker(args):
    ...
    if args.resume:
        ...
        logger.set_names(['Learning Rate', 'Train Loss', 'Val Loss', 'Val Acc.', 'Val Side1 Acc.', 'Val Side2 Acc.', 'Val Side3 Acc.', 'Val Auc.', 'Side1 Auc.', 'Side2 Auc.', 'Side3 Auc.'])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        val_total_loss, val_main_acc, val_side1_acc, val_side2_acc, val_side3_acc, m_auc, s1_auc, s2_auc, s3_auc = validate(val_loader, model,
                                                                                             criterion, args)
        lr = optimizer.param_groups[0]['lr']
        logger.append([lr, total_loss, val_total_loss, val_main_acc, val_side1_acc, val_side2_acc, val_side3_acc, m_auc, s1_auc, s2_auc, s3_auc])
...
def train(train_loader, model, criterion, writer, epoch, optimizer, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        total_loss = (out_main_loss + args.lambd * (side1_loss + side2_loss + side3_loss)) + args.theta * cam_loss + args.alpha * (
